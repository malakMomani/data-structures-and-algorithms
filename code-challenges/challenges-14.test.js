...
const screenForNames = (arr) => {
  // Solution code here...
  let localArr =[];
  let reg = /^(Mr. ||Mrs. ||Ms. ||Dr. )[A-Z][a-z]*/g;
  arr.forEach(element => {
    if(reg.test(element)){
...
let biggerThanLuke = (arr) => {
  ...
  } else
  {
    ...
    for (let index = 1; index < arr.length; index++) {
      ...
      if(parseInt(element.mass) > lukesMass) {
        localArr.push(element.name);
      }
    }
    const bigger = localArr.join(' - ');
    return bigger;
  }
};
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  let row1, row2 , row3, col1=[] , col2=[],col3=[];
...
const helpCheck = (row1, col1, row2, col2, row3, col3) =>{
  ...
  if(row1[0]!=='' && row2[0] !== '' && row3[0] !== '') {
    if(row1[0] === row2[0] && row1[0] === row3[0]) return true;
    if(row1[1] === row2[1] && row1[1] === row3[1]) return true;
    if(row1[2] === row2[2] && row1[2] === row3[2]) return true;
    if(row1[0] === row2[1] && row1[0] === row3[2]) return true;
    if(row1[2] === row2[1] && row1[2] === row3[0]) return true;
  }
  if(col1[0]!=='' && col2[0] !== '' && col3[0] !== '') {
    if(col1[0] === col2[0] && col1[0] === col3[0]) return true;
    if(col1[1] === col2[1] && col1[1] === col3[1]) return true;
    if(col1[2] === col2[2] && col1[2] === col3[2]) return true;
  }

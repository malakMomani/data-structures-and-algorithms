...
const totalSum = (input) => {
  ...
  let sum = input.map((element) => {
    return element.reduce((num, value) => {
      num = num + value;
      return num;
    }, 0);
  });
  let toatal = sum.reduce((acc, value) => {
    acc = acc + value;
    return acc;
  }, 0);
  return toatal;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  result.forEach(element=>{
    mathResult.push(element.map(number =>{
